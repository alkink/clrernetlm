...
model = dict(
    ...
    lanelm_cfg=dict(
        # Match checkpoint training (100-img run): vocab_size_x = 300, single-level (P5)
        nbins_x=300,
        max_y_tokens=41,
        embed_dim=256,
        num_layers=4,
        ...
        ffn_dim=512,
        max_seq_len=40,
        visual_in_channels=(64,),  # P5 only as trained
    ...
    tokenizer_cfg=dict(
        ...
        img_h=320,
        num_steps=40,
        nbins_x=300,        # match LM vocab (overlapping relative as trained)
...
# Override test dataloader for 100 images
test_dataloader = dict(
    batch_size=2,
    num_workers=2,
    dataset=dict(
        data_list="dataset/list/train_100.txt",  # 100 images
        test_mode=True,
    ),
)

# Use CULaneMetric evaluator
test_evaluator = dict(
    type='CULaneMetric',
    data_root='dataset',
    data_list='dataset/list/train_100.txt',
    use_parallel=False,
    result_dir='work_dirs/lanelm_100_imgs_mmtest/preds',
)

# Test config
test_cfg = dict(type='TestLoop')

# Disable visualizer for faster inference
visualizer = None
