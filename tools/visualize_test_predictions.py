#!/usr/bin/env python3
"""
Visualize LaneLM test predictions vs ground truth.
Matches the visualization logic from train_lanelm_v4_fixed.py
"""
import argparse
import os
import cv2
...
def visualize_predictions(
    ...
    save_dir, max_samples=20, max_lanes=4
):
    ...
    # Filter samples with GT (ground truth)
    # In test_mode, gt_points is empty, so we need to load GT manually
    ...
    for idx in range(len(dataset)):
        ...
        if os.path.exists(gt_path):
            ...
            if gt_data and len(gt_data) > 0:
                # Check if at least one lane has valid points
    ...
    print(f"Visualizing {num_samples} samples with GT...")
    ...
    for sample_idx_idx, (sample_idx, gt_data) in enumerate(valid_samples[:num_samples]):
        ...
        with torch.no_grad():
            # Extract features
            feats = extract_full_fpn_feats(clrernet_model, img_tensor)
            visual_tokens = model.encode_visual_tokens(feats)
            
            # CRITICAL: Use EXACT same decode logic as test script (LaneLMDetector.predict)
            # This ensures birebir same predictions
            ...
            # Convert to resized coords exactly like test script
            all_preds = []
            for l in range(x_tok.shape[0]):
                # CRITICAL: Use same decode as test script (line 274)
                coords_resized = tokenizer.decode_single_lane(x_tok[l], y_tok[l], smooth=True)
                
                # Convert to normalized Lane (like test script line 275-283)
                ...
                if lane is not None and lane.points is not None and lane.points.shape[0] >= 2:
                    # CRITICAL: Use the SAME coords_resized that was used to create the Lane
                    # This ensures birebir same as test script
                    # Test script uses coords_resized directly from decode_single_lane
                    # We already have coords_resized, so use it directly
                    all_preds.append(coords_resized)
                else:
            ...
            # Draw predictions (colored) - using same coords as test script
            colors = [(0, 0, 255), (255, 0, 0), (255, 0, 255), (0, 255, 255)]  # BGR format
            pred_count = 0
            for l_idx, coords in enumerate(all_preds):
                if coords is None or coords.shape[0] < 2:
                    continue
                
                # coords are already in resized pixel space [0, 800] x [0, 320]
                pred_count += 1
            ...
            # Draw GT (GREEN)
            # GT is in original image coordinates (pixel space), need to convert to resized space
            gt_count = 0
            if 'gt_points' in sample and sample['gt_points']:
                for lane in sample['gt_points'][:max_lanes]:
                    ...
                    # Original: (1640, 590), Crop: (0, 270, 1640, 590), Resize: (800, 320)
...
def main():
    parser = argparse.ArgumentParser(description="Visualize LaneLM test predictions")
    ...
    parser.add_argument("--data-list", default="dataset/list/test_valid_gt_100.txt")
    parser.add_argument("--work-dir", default="work_dirs/lanelm_v4_test_vis")
    parser.add_argument("--max-samples", type=int, default=20, help="Max number of samples to visualize")
    ...
    visualize_predictions(
        ...
        args.work_dir, max_samples=args.max_samples
    )
