"""
LaneLM Medium-Scale Overfit Script (100 Images) - FIX 2.

Diagnosis:
  - Loss exploded to 40,000+ because the Logit Mask was likely masking out the TRUE targets.
  - If the Ground Truth is token 150, but mask sets index 150 to -inf, loss becomes infinite.

Fixes:
  - DISABLED 'apply_logit_mask' completely. Let the model learn the distribution naturally.
  - Added Token Range Debugging (Print min/max tokens).
"""
...
def autoregressive_decode_tokens_masked(
    ...
    for lane_idx in range(max_lanes):
        ...
        for t in range(T):
            ...
            # MASKING REMOVED HERE TOO
            # if t == 0: ...
            
            # Mask only BOS tokens from OUTPUT (prevent predicting BOS again)
...
def build_clean_dataloader(data_root, list_path, batch_size, num_workers, overfit_size=100):
...
def visualize_overfit(
    ...
    with torch.no_grad():
        ...
        for i in range(min(len(imgs), 4)):
            ...
            save_path = os.path.join(save_dir, f"ep{epoch:04d}_batch_img{i}.jpg")
...
def main():
    ...
    parser.add_argument("--work-dir", default="work_dirs/lanelm_100_imgs")
    ...
    dataloader, subset = build_clean_dataloader(
        args.data_root, args.list_path, batch_size=8, num_workers=4, overfit_size=100
    )
    fixed_vis_batch = next(iter(dataloader))
    ...
    print(f"Starting 100-IMAGE Training (NO MASKING)...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # DEBUG: Print Token Stats ONCE per epoch
        debug_printed = False
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss_x = loss_fn(logits_flat, targets_flat)
            ...
            if not debug_printed:
                valid_mask = targets_flat != tokenizer_cfg.pad_token_x
                valid_targets = targets_flat[valid_mask]
                if len(valid_targets) > 0:
                    print(f"DEBUG Ep {epoch}: Targets Min={valid_targets.min().item()}, Max={valid_targets.max().item()}")
                debug_printed = True
        ...
        if avg_loss < best_loss and steps > 0:
            ...
            torch.save({
                ...
                "epoch": epoch,
                "loss": best_loss
            }, os.path.join(args.work_dir, "lanelm_100_best.pth"))
